...
def get_image_list_path(image_folder, image_score_list, n_quality_levels, do_normalization=False, score_type='mos'):
    images_scores = []
    for image_score in image_score_list:
        content = image_score.split(',')
        if len(content) > 2:
            if score_type == 'category':
                score = ','.join([x for x in content[-9:]])
            else:
...
def train_main(args):
    ...
    if args['image_folder']:
        train_images_scores = get_image_list_path(args['image_folder'], train_images_scores,
                                                  args['n_quality_levels'], do_normalization=True, score_type='score')
        val_images_scores = get_image_list_path(args['image_folder'], val_images_scores, args['n_quality_levels'],
                                                do_normalization=True, score_type='score')
        test_images_scores = get_image_list_path(args['image_folder'], test_images_scores, args['n_quality_levels'],
                                                 do_normalization=True, score_type='score')

    # Create sca_iqa_net model
