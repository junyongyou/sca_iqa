...
class ResolutionGroupGenerator(Sequence):
    ...
    """
    def __init__(self, images_scores, batch_size=16, image_aug=False, shuffle=True, imagenet_pretrain=True, score_type='mos'):
    ...
    def on_epoch_end(self):
        if self.shuffle:
            ...
            for i, resolution in enumerate(resolutions):
                ...
                self.resolution_groups.update({resolution: images_scores_per_resolution})
    ...
    def __getitem__(self, item):
        ...
        for idx_1 in range(item * self.batch_size, (item + 1) * self.batch_size):
            ...
            # image = np.asarray(Image.open(image_score[0]).resize((224, 224)), dtype=np.float32)
